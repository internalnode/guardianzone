...
const archiveData = {
  report021:{
    ...
    update:"15:42",
    image:"https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?q=80&w=1200&auto=format&fit=crop",
    caption:"caméra secondaire — relais périphérique // récupération incomplète",
    body:[
      "Transmission retrouvée sur relais secondaire.",
      "Origine non identifiée.",
      "Signal perdu après 4 secondes."
    ],
    warning:"Maintenance non signée détectée à 02:14."
  },

  fieldnote12:{
    title:"FIELD_NOTE_12",
    type:"FIELD NOTE",
    sector:"FOREST BUNKER",
    status:"ACTIVE",
    update:"03:11",
    image:"https://images.unsplash.com/photo-1448375240586-882707db888b?q=80&w=1200&auto=format&fit=crop",
    caption:"observation terrain — bunker forestier // visibilité réduite",
    body:[
      "Présence de traces récentes autour du bunker forestier.",
      "Aucune activité visuelle confirmée.",
      "Générateur brièvement relancé."
    ],
    warning:"Signal thermique faible détecté durant 9 secondes."
  },

  recovered77:{
    title:"RECOVERED_DATA_77",
    type:"RECOVERED DATA",
    sector:"SOUTH GRID",
    status:"PARTIAL ACCESS",
    update:"22:08",
    image:"https://images.unsplash.com/photo-1513828583688-c52646db42da?q=80&w=1200&auto=format&fit=crop",
    caption:"fragment récupéré — source interne inconnue",
    body:[
      "Fragment vidéo récupéré depuis archive corrompue.",
      "Une seule image exploitable retrouvée.",
      "Canal interrompu avant authentification."
    ],
    warning:"Archive partiellement déchiffrée."
  },

  sealed09:{
    title:"SEALED_ARCHIVE_09",
    type:"SEALED ARCHIVE",
    sector:"UNKNOWN",
    status:"LOCKED",
    update:"--:--",
    image:"https://images.unsplash.com/photo-1520607162513-77705c0f0d4a?q=80&w=1200&auto=format&fit=crop",
    caption:"archive verrouillée — accès refusé",
    body:[
      "Clé d’accès manquante.",
      "Structure archive endommagée.",
      "Lecture impossible depuis ce terminal."
    ],
    warning:"AUTHORIZATION LEVEL REQUIRED."
  }
};

function renderArchive(id){
  const data = archiveData[id];
  ...
  viewer.innerHTML = `
    ...
    <div class="archive-meta">
      <div><label>TYPE</label><p>${data.type}</p></div>
      <div><label>SECTOR</label><p>${data.sector}</p></div>
      <div><label>STATUS</label><p>${data.status}</p></div>
      <div><label>LAST UPDATE</label><p>${data.update}</p></div>
    </div>

    <div class="archive-photo">
      <img src="${data.image}">
      <div class="archive-photo-caption">${data.caption}</div>
    </div>

    <div class="archive-text">
      ${data.body.map(t=>`<p>${t}</p>`).join("")}
      <p class="warning-line">${data.warning}</p>
    </div>
  `;
}
...
document.querySelectorAll(".archive-entry").forEach(btn=>{
  btn.addEventListener("click", ()=>{
    document.querySelectorAll(".archive-entry").forEach(b=>b.classList.remove("active"));
    btn.classList.add("active");
    renderArchive(btn.dataset.report);
  });
});
